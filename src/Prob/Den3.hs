...
denStmt (x :~ Bernoulli theta) = P $ \s -> [(theta, (M.insert x True s, ())), (1 - theta, (M.insert x False s, ()))]
...
denStmt loop@(While e s1) = P $ \s -> if denExpr e s then (let P p = denStmts (s1++[loop]) in p s) else [(1, (s, ()))]
